...
- **`src/Common/Gdip_All_v2.ahk` 覆盖了内置 `IsNumber`/`IsInteger`**：该库末尾自定义了这两个同名函数（v2 `is number` 语义，只认数字类型，不认 `"09"` 这类数字字符串），会静默覆盖 AHK 内置版本。因此**判断「字符串是不是数字」必须用正则（项目内封装为 `IsIntText()`），不要用 `IsNumber()`**；否则在真实运行环境（加载了 Gdip）里判断恒为假，而只加载 Config 的探针/单测里却正常，极易误判。曾导致设置窗口「保存设置总是报清空时刻格式错误」与 `config.ini` 整数配置被静默回退默认值。
