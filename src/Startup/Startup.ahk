...
; 快捷方式创建/删除与桌面快捷方式共用 Common\Shortcut.ahk（P1-4 去重）
; ==================================================================
#Include "..\Common\Shortcut.ahk"
...
IsStartupEnabled() {
    return ShortcutExists(StartupShortcutPath())
}

; 创建开机启动快捷方式（已存在时跳过；失败记录日志但不中断脚本）
CreateStartupShortcut() {
    ShortcutCreateIfMissing(StartupShortcutPath(), "开机启动")
}

; 删除开机启动快捷方式（不存在时跳过；失败记录日志但不中断脚本）
RemoveStartupShortcut() {
    ShortcutRemoveIfExists(StartupShortcutPath(), "开机启动")
}
