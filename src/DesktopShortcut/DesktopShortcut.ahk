...
; 创建/删除与开机启动共用 Common\Shortcut.ahk（P1-4 去重）
; ==================================================================
...
IsDesktopShortcutCreated() {
    return ShortcutExists(DesktopShortcutPath())
}

; 创建桌面快捷方式（已存在时跳过；失败记录日志但不中断脚本）
CreateDesktopShortcut() {
    ShortcutCreateIfMissing(DesktopShortcutPath(), "桌面快捷方式")
}

; 删除桌面快捷方式（不存在时跳过；失败记录日志但不中断脚本）
RemoveDesktopShortcut() {
    ShortcutRemoveIfExists(DesktopShortcutPath(), "桌面快捷方式")
}
