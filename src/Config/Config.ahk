...
; ==================================================================
; 滚动截图参数（手动滚动 + 自动抓帧拼接；由 F1 选区工具栏「滚动截图」进入）
; ==================================================================
SCROLL_POLL_MS      := 120     ; 画面变化轮询间隔（毫秒）：越小越跟手，越大越省 CPU
SCROLL_TIMEOUT_MS   := 180000  ; 滚动截图安全超时（毫秒），防止长时间停留卡住无法退出
SCROLL_TIP_GAP      := 8       ; 提示条与选区边距（像素）
SCROLL_TIP_BG       := UI_BG   ; 提示条背景色
SCROLL_TIP_TEXT     := UI_TEXT ; 提示条文字色（与提示条底色对应）
SCROLL_TIP_HINT     := "滚动截图：向下滚动内容，完成后按 Esc 或右键结束"
; ==================================================================
...
; 为何不直接用内置 IsNumber()/IsInteger()：它们接受 "0x1A"、"1.5"、"1e3" 等非十进制整数写法，
; 而配置项只想要「可选负号的十进制整数」；且历史上 Gdip 库曾同名覆盖过内置版本
; （现已改名 Gdip_IsNumber/Gdip_IsInteger 修复），统一用本函数判定更稳。
; 该问题曾造成：设置窗口每次保存都报「清空时刻格式错误」、ini 里的整数配置被静默替换成默认值。
IsIntText(s) {
